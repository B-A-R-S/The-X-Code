...
export default function MealRecommendation() {
  ...
  const calculateServings = async () => {
    ...
    const input = [Number(age), Number(gender), Number(activityLevel), Number(weight), Number(height), Number(recCal), Number(recTotCarb), Number(mealType), Number(recCarb) ,Number(menuNo)];
    
    try {
        const response = await axios.post('https://us-central1-t1-diabetes-management-68efb.cloudfunctions.net/predict_meal', { "int_array": input });
        setYNew(response.data);
